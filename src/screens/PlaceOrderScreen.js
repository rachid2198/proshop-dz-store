...
import {createOrder, clearOrder} from '../features/OrderSlice'
import { clearCart } from '../features/CartSlice'

const PlaceOrderScreen = () => {
  const history=useNavigate()  
  const {order,error,success}= useSelector((store)=>store.order) 
  const cart= useSelector((store)=>store.cart)
  const dispatch=useDispatch()  

  const itemsPrice= cart.cartItems.reduce((acc,item)=>acc + item.price * item.qty,0)
  const shippingPrice= (itemsPrice>100 ? 0:10).toFixed(3)
